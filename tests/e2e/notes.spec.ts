...
test.describe('Notes', () => {
  ...
  test('should create a new note', async ({ page }) => {
    ...
    await page.locator('.tiptap').fill('This is a test note')
  ...
  test('should delete a note', async ({ page }) => {
    ...
    await page.locator('.tiptap').fill('This note will be deleted')
    await page.click('button[type="submit"]')
    await page.waitForURL(/\/notes\//)
  ...
  test('should create a note with all types', async ({ page }) => {
    ...
    for (const type of types) {
      ...
      await page.locator('.tiptap').fill(`This is a ${type} note`)
      await page.click('button[type="submit"]')
      await page.waitForURL(/\/notes\//)
  ...
  test('should handle private notes', async ({ page }) => {
    ...
    await page.locator('.tiptap').fill('This is private')
    await page.click('button[type="submit"]')
    await page.waitForURL(/\/notes\//)
  ...
  test('should display note details correctly', async ({ page }) => {
    ...
    await page.locator('.tiptap').fill('Detailed content about lore')
    await page.click('button[type="submit"]')
    await page.waitForURL(/\/notes\//)

    await expect(page.locator('h1')).toContainText('Detailed Note')
    await expect(page.locator('text=Lore')).toBeVisible()
    await expect(page.locator('text=Detailed content about lore')).toBeVisible()
  })
  ...
  test('should handle image uploads', async ({ page }) => {
    ...
    await page.locator('.tiptap').fill('Note content')
    await page.click('button[type="submit"]')
    await page.waitForURL(/\/notes\//)
  ...
  test('should navigate back to notes list', async ({ page }) => {
    ...
    await page.locator('.tiptap').fill('Test content')
    await page.click('button[type="submit"]')
    await page.waitForURL(/\/notes\//)

    await page.click('text=Back to Notes')
    await expect(page).toHaveURL(new RegExp(`/notes\\?campaignId=${campaignId}`))
  })
