...
test.describe('Character Management', () => {
  ...
  test.skip('should display empty state when no characters exist', async ({ page }) => {
  ...
  test.skip('should show campaign requirement warning', async ({ page }) => {
    ...
    await expect(page.getByText('Please select a campaign to view characters')).toBeVisible()
  })
  ...
  test.skip('should format dates correctly', async ({ page }) => {
    ...
    await expect(dateCell).toContainText(/Today|Yesterday|\d+ days ago|\d{1,2}\/\d{1,2}\/\d{4}/)
  })
})
